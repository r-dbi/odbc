...
void query::execute()
{
	...
	if (columns != 0) {
		if (use_double_buffering_) {
			results_ = boost::make_shared<result_sets::double_buffered_result_set>(statement_, buffer_size_);
		} else {
			results_ = boost::make_shared<result_sets::bound_result_set>(statement_, buffer_size_);
		}
	}
}
