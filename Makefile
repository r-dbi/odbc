...
cpp11:
	...
	clang++ example.cpp nanodbc.cpp -o example $(iodbc-config --libs) ${ODBC_FLAGS} -Wall -g -std=c++11 -DNANODBC_USE_CPP11

gnu:
	# Example build for g++ using iODBC on OS X
	g++ example.cpp nanodbc.cpp -o example $(iodbc-config --libs) ${ODBC_FLAGS} -Wall -g --stdlib=libstdc++
