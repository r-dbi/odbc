...
Next, you'll install the Basic, ODBC, and SQLPlus Instant Client libraries from the [Oracle website](https://www.oracle.com/database/technologies/instant-client/macos-arm64-downloads.html) by:

* Downloading the Basic, ODBC, and SQLPlus `.dmg`s
* Creating a folder for the instant client to live in. I made mine at `/Users/simoncouch/instantclient_23_3`. (This folder will show up many times in the rest of these instructions--always substitute it with your own from here on out.)
* For each of the `dmg`s, opening up the image and copying and pasting its contents into the folder you made in the previous step.
* Symlink your `libodbcinst.2.dylib` into the folder. I used `ln -s /opt/homebrew/Cellar/unixodbc/2.3.12/lib/libodbcinst.2.dylib /Users/simoncouch/instantclient_23_3/libodbcinst.2.dylib` to do so for myself; use `odbc_config --lib-prefix` to help locate yours.
...
Now, set some paths to help RStudio and the Instant Client communicate. In `usethis::edit_r_profile()`, add:
...
In the RStudio terminal, test your Instant Client installation and database connection with:
...
* Setting those DBQ, UID, and PWD parameters in my `.ini` file instead of the R session introduces errors for me. 
